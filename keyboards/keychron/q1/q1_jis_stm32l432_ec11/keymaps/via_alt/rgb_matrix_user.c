...
void rgb_matrix_indicators_advanced_user(uint8_t led_min, uint8_t led_max) {
    ...
    switch (current_layer) {
        ...
        case WIN_BASE:
            ...
            if (get_light_wasd()) {
                rgb_matrix_set_color_by_keycode(led_min, led_max, current_layer, is_wasd, CAPS_LOCK_INDICATOR_COLOR);
            }
...
bool is_wasd(uint16_t keycode) {
    bool indicator = keycode == KC_W || keycode == KC_A || keycode == KC_S || keycode == KC_D;
    return indicator;
}
